...
from .controllers.sensor_moisture_data_controller import sensor_moisture_data_blueprint
from .controllers.sensor_temperature_data_controller import sensor_temperature_data_blueprint
...
# Configuration for Celery with Redis
app.config['CELERY_BROKER_URL'] = os.getenv('REDIS_URL')
app.config['CELERY_RESULT_BACKEND'] = os.getenv('REDIS_URL')

# Database configuration
app.config[
    'SQLALCHEMY_DATABASE_URI'] = f"mysql+pymysql://{os.getenv('MYSQL_USER')}:{os.getenv('MYSQL_PASSWORD')}@{os.getenv('SERVICE_IP')}/{os.getenv('MYSQL_DATABASE')}"
...
app.register_blueprint(sensor_moisture_data_blueprint, url_prefix='/api')
app.register_blueprint(sensor_temperature_data_blueprint, url_prefix='/api')
